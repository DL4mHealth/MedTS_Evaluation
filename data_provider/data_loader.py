...
class RandomDependentLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, 'Feature/')
        self.label_path = os.path.join(root_path, 'Label/label.npy')
        ...
        # load data in subject-dependent manner
        self.X, self.y = self.load_random(self.data_path, self.label_path, flag=flag)
    ...
    def load_random(self, data_path, label_path, flag=None):
        '''
        Loads data from npy files in data_path based on flag and ids in label_path,
        but randomly shuffle the label and keep the subject ID.
        This makes all samples in the same subject has the same labels, but the label is incorrect.
        Args:
            data_path: directory of data files
            label_path: directory of label.npy file
            flag: 'train', 'val', or 'test'
        ...
        for i in range(len(subject_label)):
            subject_label[i][0] = random.choice(sorted(set(subject_label[:, 0])))
...
class SubjectLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        ...
        # load data in subject-dependent manner
        self.X, self.y = self.load_subject(self.data_path, self.label_path, flag=flag)
    ...
    def load_subject(self, data_path, label_path, flag=None):
        '''
        Loads data from npy files in data_path based on flag and ids in label_path,
        but randomly shuffle the label and keep the subject ID.
        This makes all samples in the same subject has the same labels, but the label is incorrect.
        ...
        # The first column is the label; the second column is the patient ID
        subject_label = np.load(label_path)
        # subject ID start from 1, subtract 1 to make it from 0
        subject_label[:, 1] = subject_label[:, 1] - 1
        for filename in os.listdir(data_path):
            filenames.append(filename)
        filenames = natsorted(filenames)
        # print(filenames)
        for j in range(len(filenames)):
            ...
            path = data_path + filenames[j]
            subject_feature = np.load(path)
            X_train, X_val = train_test_split(subject_feature, test_size=0.2, random_state=42)
            X_train, X_test = train_test_split(X_train, test_size=0.25, random_state=42)
            # num = subject_feature.shape[0]
            if flag == 'TRAIN':
                subject_feature = X_train
            elif flag == 'VAL':
                subject_feature = X_val
            elif flag == 'TEST':
                subject_feature = X_test
            else:
                raise Exception('flag must be TRAIN, VAL, or TEST')
            for trial_feature in subject_feature:
                feature_list.append(trial_feature)
        ...
        return np.array(feature_list), np.array(label_list)[:, 1]
...
class TDBRAINIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
        ...
        # pre_process
        # self.X = bandpass_filter_func(self.X, fs=256, lowcut=0.5, highcut=100)
        self.X = normalize_batch_ts(self.X)
...
class TDBRAINRandomIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
...
class ADFTDIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
...
class ADFTDRandomIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
...
class PTBRandomIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
...
class PTBXLIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
    ...
    def load_train_val_test_list(self, label_path, a=0.6, b=0.8, c=1.0):
        """
        Loads IDs for training, validation, and test sets
        Args:
            label_path: directory of label.npy file
        ...
        test_ids = (
            no_list[int(b * len(no_list)) : int(c * len(no_list))]
            + mi_list[int(b * len(mi_list)) : int(c * len(mi_list))]
            + sttc_list[int(b * len(sttc_list)) : int(c * len(sttc_list))]
            + cd_list[int(b * len(cd_list)) : int(c * len(cd_list))]
            + hyp_list[int(b * len(hyp_list)) : int(c * len(hyp_list))]
        )
...
class PTBXLRandomIndepLoader(Dataset):
    def __init__(self, args, root_path, flag=None):
        self.root_path = root_path
        self.data_path = os.path.join(root_path, "Feature/")
        self.label_path = os.path.join(root_path, "Label/label.npy")
